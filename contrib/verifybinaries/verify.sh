...
###   This script attempts to download the signature file SHA256SUMS.asc from kekcoin.org
...
WORKINGDIR="/tmp/kekcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://kekcoin.org/bin/"
VERSIONPREFIX="kekcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kekcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the KekCoin Core binary release signing key installed?"
   fi
