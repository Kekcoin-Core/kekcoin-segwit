...
The RPM spec file provided here is for KekCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kekcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[KekCoin git repository](https://github.com/kekcoin/kekcoin) but are not part of
...
Currently only `Source10` is used, for the example `kekcoin.conf` file.
...
in the kekcoin git repository. At present only an SVG version of the KekCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kekcoin
...
needed when 0.12.1 is released, a proper fix is already in the KekCoin git
master branch.
...
idea to build KekCoin Core against a static version of that library compiled
...
The `kekcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kekcoin-server` package. At this point, most RPM based
...
The `kekcoin-server` package should have SELinux support. How to properly do
...
KekCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kekcoin/). He has been
packaging KekCoin for Fedora at least since 2012.
...
4. I renamed the `kekcoin` package that contains the Qt GUI to `kekcoin-core` as
that appears to be how the general population refers to it, in contrast to
`kekcoin-xt` or `kekcoin-classic`. I wanted to make sure the general population
