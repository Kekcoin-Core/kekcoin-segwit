...
int nTargetSpacing = 60; // Blocktime: 60 secs
...
{
    LOCK(cs_main);
    return false; //(VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_SEGWIT, versionbitscache) == THRESHOLD_ACTIVE);
}

bool IsWitnessLocked(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    LOCK(cs_main);
    return false; //(VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_SEGWIT, versionbitscache) == THRESHOLD_LOCKED_IN);
}
...
static int64_t nTargetTimespan = 22 * 60;  // 12.5 mins
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    ...
    else if(nHeight >= 101 && nHeight <= 9999)
        {
            nSubsidy = 50 * COIN;
        }
    
    else if(nHeight >= 10000 && nHeight <= 19999)
        {
            nSubsidy = 25 * COIN;
        }
    ...
    else if(nHeight >= 30000 && nHeight <= 99999)
        {
            nSubsidy = 5 * COIN;
        }

    else if(nHeight >= 100000 && nHeight <= 199999)
        {
            nSubsidy = 2.5 * COIN;
        }

    else if(nHeight >= 200000 && nHeight <= 9770000)
        {
            nSubsidy = 1 * COIN;
        }
