 ...
 * between kekcoind, and kekcoin-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), KEKCOIN_CONF_FILENAME));
    if (mode == HMM_KEKCOIND)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), KEKCOIN_PID_FILENAME));
#endif
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://kekcoin.co>";
...
/** Sanity checks
 *  Ensure that KekCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single KekCoin process is using the data directory.
    ...
    if(GetBoolArg("-staking", true))
        threadGroup.create_thread(boost::bind(&KekCoinStaker, boost::cref(chainparams)));
#endif
