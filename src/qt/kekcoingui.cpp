...
const QString KekCoinGUI::DEFAULT_WALLET = "~Default";

KekCoinGUI::KekCoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void KekCoinGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/kekcoin"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a KekCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kekcoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your KekCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified KekCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a kekcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible KekCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void KekCoinGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(KekCoinUnits::Unit u, KekCoinUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(KekCoinUnits::name(u)), this);
...
void KekCoinGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void KekCoinGUI::createToolBars()
{
    if(walletFrame)
    {
...
void KekCoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool KekCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool KekCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void KekCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KekCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void KekCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void KekCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KekCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KekCoinGUI::optionsClicked()
{
...
void KekCoinGUI::showDebugWindow()
{
...
void KekCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void KekCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void KekCoinGUI::openClicked()
{
...
void KekCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void KekCoinGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KekCoinGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KekCoinGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KekCoinGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KekCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KekCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void KekCoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
...
void KekCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
...
void KekCoinGUI::showVotingDialog()
{
...
void KekCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("KekCoin"); // default title
...
void KekCoinGUI::changeEvent(QEvent *e)
{
...
void KekCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KekCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KekCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KekCoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void KekCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KekCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KekCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KekCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KekCoinGUI::setEncryptionStatus(int status)
{
...
void KekCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KekCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KekCoinGUI::detectShutdown()
{
...
void KekCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void KekCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(KekCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KekCoinGUI::subscribeToCoreSignals()
{
...
void KekCoinGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<KekCoinUnits::Unit> units = KekCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const KekCoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(KekCoinUnits::name(unit)));
    }
...
{
    setText(KekCoinUnits::name(newUnits));
}
...
void KekCoinGUI::updateStakingStatus()
{
    updateWeight();
