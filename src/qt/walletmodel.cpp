...
{
    CKekCoinAddress addressParsed(address.toStdString());
        ...
        Q_FOREACH(const CRecipient &rcp, transaction.vecSend)
        {
          ...
          Q_FOREACH(const CWalletTx newTx, transaction.vTransactions)
          {
            ...
            Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
            {
              ...
              else if (!rcp.message.isEmpty()) // Message from normal kekcoin:URI (kekcoin:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CKekCoinAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CKekCoinAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CKekCoinAddress(sAddress).Get();
